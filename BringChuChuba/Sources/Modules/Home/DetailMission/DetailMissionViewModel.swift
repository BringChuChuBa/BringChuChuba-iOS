    ...
    func transform(input: Input) -> Output {
        ...
        let mission = input.contractTrigger.flatMapLatest {
            return Network.shared.request(
                with: .contractMission(missionUid: Int(self.mission.id)!),
                for: Mission.self
            )
        ...
        let items = mission.map { mission -> [DetailMissionSection] in
            ...
            // title
            let titleCellViewModel = DetailMissionCellViewModel(
                with: "Common.Title".localized,
            ...
            if let description = mission.description {
                let descriptionCellViewModel = DetailMissionCellViewModel(
                    with: "Common.Description".localized,
                    detail: description
                )
            ...
            // reward
            let rewardCellViewModel = DetailMissionCellViewModel(
                with: "Common.Reward".localized,
                detail: mission.reward
            )
            items.append(DetailMissionSectionItem.rewardItem(viewModel: rewardCellViewModel))

            // client
            let clientCellViewModel = DetailMissionCellViewModel(
                with: "Common.Client".localized,
                detail: mission.client.id // nickname
            )
            items.append(DetailMissionSectionItem.clientItem(viewModel: clientCellViewModel))

            // contractor
            if let contractor = mission.contractor {
                let contractorCellViewModel = DetailMissionCellViewModel(
                    with: "Common.Contractor".localized,
                    detail: contractor.id // nickname
                )
                items.append(DetailMissionSectionItem.contractorItem(viewModel: contractorCellViewModel))
            }

            // expireAt
            let expireAtCellViewModel = DetailMissionCellViewModel(
                with: "Common.ExpireAt".localized,
                detail: mission.expireAt
            )
            items.append(DetailMissionSectionItem.expireAtItem(viewModel: expireAtCellViewModel))

            // status
            let statusCellViewModel = DetailMissionCellViewModel(
                with: "Common.Status".localized,
                detail: mission.status.rawValue
            )
